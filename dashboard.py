...
def local_cdss_rule_engine(s_text: str, o_text: str) -> list[dict]:
    ...
    if sum(1 for kw in napas_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in gas_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in pola_napas_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in aspirasi_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    cardiac_kw = ["penurunan curah jantung", "jantung", "orthopnea", "pnd", "edema",
                  "jvp", "murmur", "ef menurun", "cardiomegali", "nyeri dada",
                  "infark", "chf", "gagal jantung", "aritmia", "palpitasi",
                  "bradikardia", "takikardia", "td menurun", "hipotensi", "syok kardiogenik"]
    if sum(1 for kw in cardiac_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in perifer_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in mio_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in perdarahan_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in serebral_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in hipovolemia_kw if kw in combined) >= 2 or is_dehydration:
        rekomendasi.append({
    ...
    if sum(1 for kw in hipervolemia_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in nutrisi_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in gula_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in urin_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in konstipasi_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in diare_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in aktivitas_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in tidur_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in keletihan_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in komunikasi_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in konfusi_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in nyeri_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in nyeri_kronis_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in ansietas_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in kulit_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in infeksi_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in jatuh_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in pascabedah_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in syok_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in vent_spontan_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in weaning_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in rosc_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in risiko_rosc_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in risiko_cj_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in serebral_aktual_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in pe_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in kardiopulmonal_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in kalium_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in natrium_kw if kw in combined) >= 2:
        rekomendasi.append({
            "kode_diagnosa": "D.0037",
            "diagnosa_keperawatan": (
    ...
    if sum(1 for kw in mg_phos_kw if kw in combined) >= 2:
        rekomendasi.append({
            "kode_diagnosa": "D.0037",
            "diagnosa_keperawatan": (
    ...
    if sum(1 for kw in acid_base_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in hipertermia_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in hipotermia_kw if kw in combined) >= 2:
        rekomendasi.append({
            "kode_diagnosa": "D.0131",
            "diagnosa_keperawatan": (
    ...
    if sum(1 for kw in sepsis_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in mobilitas_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in ambulasi_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in nyeri_cardiac_kw if kw in combined) >= 2:
        rekomendasi.append({
            "kode_diagnosa": "D.0077",
            "diagnosa_keperawatan": (
    ...
    if sum(1 for kw in citra_kw if kw in combined) >= 2:
        rekomendasi.append({
    ...
    if sum(1 for kw in hopeless_kw if kw in combined) >= 2:
        rekomendasi.append({
...
def call_cdss_api(s_input: str, o_input: str) -> dict | None:
    """
    Kirim data ke FastAPI backend untuk deteksi nilai kritis & rekomendasi 3S.
    Return None jika API tidak dapat dihubungi atau response tidak valid.
    """
    combined_text = f"Subjektif: {s_input}. Objektif: {o_input}."
    payload = {"clinical_note": combined_text}
    try:
        res = requests.post(LAB_API_URL, json=payload, timeout=API_TIMEOUT)
        res.raise_for_status()
        data = res.json()
        if not isinstance(data, dict):
            logger.warning("API response CDSS bukan dict: %s", type(data))
            return None
        return data
    except Exception as exc:
        logger.error("Gagal menghubungi API CDSS: %s", str(exc))
        return None


def formulasikan_asuhan(s_input: str, o_input: str) -> tuple[list[dict], str]:
    """
    Jalankan pipeline hybrid CDSS.
    Kembalikan (daftar_asuhan, sumber) di mana sumber = "API" atau "Lokal".
    """
    api_result = call_backend_api(s_input, o_input)
    if api_result is not None:
        return api_result, "API"
    lokal_result = local_cdss_rule_engine(s_input, o_input)
    return lokal_result, "Lokal"
...
def main_app() -> None:
    ...
    if st.button("Formulasikan Standar 3S", type="primary"):
        if not s_input.strip() and not o_input.strip():
            ...
            with st.spinner("Menganalisis data klinis..."):
                data_asuhan, sumber = formulasikan_asuhan(s_input, o_input)
    ...
    if st.session_state.daftar_asuhan:
        # Tampilkan alert CDSS
        cdss_data = st.session_state.get("hasil_cdss")
        if cdss_data and cdss_data.get("analisis"):
